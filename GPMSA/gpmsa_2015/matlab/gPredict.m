  ...
  switch mode
    ...
    case 'uvpredNoblock'
        pred=uvPred(xpred,pvals,model,data,theta,addResidVar, ...
                    returnRealization,returnMuSigma,returnCC);
    case 'epred'
        pred=epred(xpred,pvals,model,data,theta);
    otherwise
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function pred=epred(xpred,pvals,model,data,thetapred)
% write down codes for emode
    n=model.n; m=model.m;p=model.p;q=model.q;pu=model.pu;
        
    npred=size(xpred,1);
    
    % loops required: 
    % (1) # of prediction points
    % (2) for each prediction # of basis loadings
    % (3) for each loadings # of pvals
    
    %pvec=round(linspace(900,10900,50)); %to be deleted later
    %pvals=pout.pvals(pvec);
    
    npvals=length(pvals);
    pred.u=zeros(npvals,pu,npred);
    
    LamSim=model.LamSim;    % change later
    
    % new w* prediction
    for ii=1:npred             %loop over prediction points
        xpredt = [xpred(ii,:) thetapred(ii,:)];
        
        %xDist=genDist(data.zt,data.catInd);  
        zxpredDist=genDist2(data.zt,xpredt,data.catInd);
        
        for jj=1:pu            %loop over basis
            bStart=(jj-1)*(q+p)+1;
            bEnd=bStart+(q+p)-1;
            
            for kk=1:npvals    %loop over pvals
                betaU=pvals(kk).betaU(bStart:bEnd);
                lamWs=pvals(kk).lamWs(jj);
                lamWOs=pvals(kk).lamWOs;
                lamUz=pvals(kk).lamUz(jj);
                
                Sig=kron(eye(m), 1/LamSim(jj))./(lamWOs);
                SigW=gCovMat(model.ztDist,betaU,lamUz)./lamWs;
                SigWp=0;
                SigWWp=gCovMat(zxpredDist,betaU,lamUz)./lamWs;
                ...
                pred.u(kk, jj, ii)=rmultnormsvd(1,Myhat,Syhat); 
            end
        end
    end
end
